...
int process_request(TCPRequestChannel *rc, char* _request)
{
	...
	// new channel request will never happen
...
int main(int argc, char *argv[]){
	...
	while ((opt = getopt(argc, argv, "m:r:")) != -1) {
		switch (opt) {
			...
			case 'r':
				portNum = optarg;
				cout << "Port number on server side is: " << portNum << endl;
				break;
		}
	}
	srand(time_t(NULL));
	...
	TCPRequestChannel* chan = new TCPRequestChannel ("", portNum);

	cout << "Server is ready and accepting connections..." << endl;

	// connection storage
	struct sockaddr_storage their_addr;
	socklen_t addr_size;

	// loop that accepts all connections
	while (true){
		
		// accept request
		addr_size = sizeof(their_addr);

		int a_sock = accept(chan->getfd(), (struct sockaddr *)&their_addr, &addr_size);
		...
		// create a new channel with the accept socket
		TCPRequestChannel* newChan = new TCPRequestChannel(a_sock);

		// thread that calls process_request(newChan, buffer);
		thread (handle_process_loop, newChan).detach();
			// this thread will read and write using the accept socket
			// detached thread will destroy on completion
	}
